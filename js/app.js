...
fetch('data/productos.json')
  ...
  .then(productos => {
    ...
    btnConfirmarInicial.addEventListener("click", () => {
      ...
      if (!valido) return alert("Revisá que todos los valores sean números válidos");
...
function mostrarSeccionIngresos(productosIniciales) {
  ...
  btnConfirmar.addEventListener("click", () => {
    ...
    if (!valido) return alert("Ingresá solo números válidos");

    localStorage.setItem("stockTotal", JSON.stringify(stockTotal));
    alert("Stock total calculado y guardado");
...
function mostrarSeccionVentas(productosTotales) {
  ...
  btnAuditar.addEventListener("click", () => {
    ...
    if (!valido) return alert("Revisá que las ventas sean números válidos");
